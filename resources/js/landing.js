...
class AnimatedImage extends Object3D {
  constructor(source) {
    ...
    // plane
    var plane = new THREE.Mesh(new THREE.PlaneGeometry(0.77, 0.87),img);
    plane.overdraw = true;
    this.add(plane);
...
setTimeout(() => {
  const img = new AnimatedImage('/resources/img/logo.png');
...
setTimeout(() => {
  const text = new AnimatedText('Effect Voice', fontAsset);
...
setTimeout(() => {
  const text = new AnimatedText('Work from anywhere at anytime with your voice', fontAsset, 0.125);
  text.position.x -= text.basePosition * 0.5;
  text.position.y -= 0.8;
  webgl.add(text);
  text.show();
